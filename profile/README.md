...
```text
u27-check -> Stack Engine -> Context Engine -> Proof Ledger -> Boundary Engine -> Handoff Engine
```
...
The sequence is simple by design: define the system, prepare the context, write the handoff, record the proof, constrain the claim, then check the launch surface.
